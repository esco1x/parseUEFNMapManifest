...
async function reassemble(chunkData, chunksDir, outDir, decryptOpts) {
  ...
  for (const file of fileManifestList) {
    ...
    if (parts.length === 0) {
      ...
      for (const part of parts) {
        const { buffer, missing } = getPartBuffer(part);
        if (missing) missingGuids.push(part.guidHex);
        if (!ws.write(buffer)) {
  ...
  try {
    ...
    if (re != null) {
      ...
      if (re.partialFiles?.length) {
        ...
        console.log('  (Missing chunk ranges were zero-filled; content in those ranges is invalid.)');
        if (opts.debug) {
...
async function main() {
  ...
  console.log('Output folder:', path.resolve(outDir));
  console.log('Reassembling files...');
  const re = await reassemble(chunkData, chunksDir, outDir);
  const total = re.written + re.partial;
  console.log(`  ${total} file(s) written -> ${outDir}${re.partial ? ` (${re.written} complete, ${re.partial} partial)` : ''}`);
  if (re.skipped > 0) console.log(`  ${re.skipped} skipped (no chunk parts)`);
  if (re.partialFiles?.length) {
